...
extension Flight { // should probably be Identifiable & Comparable
    ...
    static func update(from faflight: FAFlight, in context: NSManagedObjectContext) -> Flight {
        ...
        flight.origin = Airport.getOrCreateByIcao(faflight.origin, context: context)
        flight.destination = Airport.getOrCreateByIcao(faflight.destination, context: context)
