    ...
    init(directory: URL) {
        ...
        do {
            ...
            for doc in documents {
                let emojirArtDocument = EmojiArtDocument(url: directory.appendingPathComponent(doc))
                self.documentNames[emojirArtDocument] = doc
            }
        } catch {
            print("create store wrong \(directory): error\(error.localizedDescription)")
        }
    }
